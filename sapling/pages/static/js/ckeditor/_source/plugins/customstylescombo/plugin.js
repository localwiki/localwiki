...
This is the -exact- same plugin as the usual stylescombo provided by
ckeditor, but we replicate it here in order to avoid a really annoying
load order issue.  We want the styles menu to carry the pages' native
CSS styling, but our inheritcss plugin (being external) runs /after/
the stylescombo plugin.  So by breaking this out into its own plugin,
with an enforced load order, we skirt around this.
*/

(function()
{
	CKEDITOR.plugins.add( 'customstylescombo',
	{
