...
/**
 * Map is used to store DOM tag attribute names and values. This map pays no
 * attention to sequence of attributes.
 * 
 * @author karol
 */
public class AttributesMap extends HashMap<String, String> {
	...
	/**
	 * constant for style attribute name, We need to treat this attribute
	 * differently to consider HTML elements equal if they only differ in style
	 * rules order (in order of style rules inside style attribute value)<br>
	 */
	protected static final String STYLE_ATTR = "style";

	/**
	 * constant for class attribute name. We need to treat this attribute
	 * differently to consider HTML elements equal if they only differ in order
	 * of multiple CSS classes inside the class attribute value.
	 */
	...
	/**
	 * just a constant to use to ignore symbols that do not affect real
	 * attributes value
	 */
	...
	/**
	 * this method returns true, if two maps have the same set of keys and
	 * values assigned to these keys. Or if the difference is only in the
	 * "style" or "class" attributes values, which are however equivalent. For
	 * the "class" attribute that means the values consist of the same set of
	 * classes, but not necessarily in the same order, and for the "style"
	 * attribute that means that the values consist of the same rules (css
	 * property : value pairs) but without order consideration.
	 */
	@Override
			...
			if (size() == attributesMap.size()) {
				...
				for (String attrib : keySet()) {
					...
					if (externalValue == null
							|| !externalValue.equals(localValue)) {
	...
	/**
	 * Checks if 2 values for "style" attribute of an HTML tag are equivalent
	 * (contain same CSS property : value pairs, but in different (or the same)
	 * order. Pairs are separated by semicolons and any amount of space, and
	 * names from values are separated by colon and any amount of space
	 * 
	 * @param style1
	 * @param style2
	 * @return
	...
	public static boolean equivalentStyles(String style1, String style2) {
		...
		final String COLON_W_SPACES = SPACE + "*+:" + SPACE + "*+";
		final String COLON = ":";
		for (int i = 0; i < styleRules1.length; i++) {
			styleRules1[i] = styleRules1[i].replaceFirst(COLON_W_SPACES, COLON);
			styleRules2[i] = styleRules2[i].replaceFirst(COLON_W_SPACES, COLON);
	...
	/**
	 * Checks if 2 values for "class" attribute of an HTML tag are equivalent
	 * (contain same CSS class names, but in different (or same order) Ignores
	 * new line symbols and tabulation<br>
	 * Example:<br>
	 ...
	 * <code>equals</code> method, so we need a way to get the styles attribute
	 * value in the same representation we use when we're comparing. We could
	 * use this method in comparison method, however the way comparison method
	 * is written now is much faster, because it can fail or succeed long before
	...
	/**
	 * The <code>hashCode()</code> method should correspond to
	 * <code>equals</code> method, so we need a way to get the class attribute
	 * value in the same representation we use when we're comparing. We could
	 * use this method in comparison method, however the way comparison method
	 * is written now is much faster, because it can fail or succeed long before
	 * normalization is finished.
	 * 
	 * @param classVal
	 *            - value of "class" attribute of an HTML tag.
	 * @return normalized representation of the provided value
	 */
