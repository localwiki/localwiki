...
public abstract class LCS {
	 ...
	 * @param V
	 *            should be allocated as int[2][l1.length + l2.length + 1], used
	 *            to store furthest reaching D-paths
	 * @param snake
	 *            should be allocated as int[3], used to store the beginning x,
	 *            y coordinates and the length of the latest snake traversed
	 * @param subMonitor
	 * @param lcs
	 *            should be allocated as TextLine[2][l1.length], used to store
	 ...
	 * @param snake
	 *            should be allocated as int[3], used to store the beginning x,
	 *            y coordinates and the length of the middle snake
	 * 
	...
	private int find_middle_snake(int bottoml1, int topl1, int bottoml2,
		...
		snake[2] = 0;
		return 5; /*
				 * HACK: since we didn't really finish the LCS computation we
				 * don't really know the length of the SES. We don't do anything
				 * with the result anyway, unless it's <=1. We know for a fact
				 * SES > 1 so 5 is as good a number as any to return here
				 */
	}
	 ...
	 * @param limit
	 *            the number of steps made in an attempt to find the LCS from
	 *            the front and back
	 * @param V
	 *            the array storing the furthest reaching D-paths for the LCS
	 *            computation
