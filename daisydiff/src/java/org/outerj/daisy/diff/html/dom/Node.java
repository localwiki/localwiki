...
public abstract class Node {
	 ...
	 * created node, but also appends the created node to the collection of the
	 * parent's children.
	 * 
	 * @param parent
	...
	/**
	 * this recursive method returns list of the ancestors that is ordered
	 * starting from the root by the depth. Index of an element in that list
	 * corresponds its depth (if depth of the root is 0)
	 * 
	 ...
	 * nodes are from the same tree, and in that case return true only if it's
	 * the same object This method returns the root of the tree (which should be
	 * common ancestor for every node in the tree). If the roots are the same
	 * object, then the nodes are in the same tree.
	 * 
	 * @return the "top" ancestor if this node has a parent,<br>
	 *         or<br>
	 *         the node itself if there is no parent, and this is a
	 *         <code>TagNode</code><br>
	 *         or<br>
	 *         null if there is no parents and this node isn't a
	 *         <code>TagNode</code>
	 */
	...
	/**
	 * Descent the ancestors list for both nodes stopping either at the first
	 * no-match case or when either of the lists is exhausted.
	 * 
	 * @param other
	 *            - the node to check for common parent
	 * @return result that contains last common parent, depth, index in the list
	 *         of children of the common parent of an ancestor(or self) of this
	 *         node that is immediate child of the common parent.
	...
	/**
	 * changes the parent field of this node. Does NOT append/remove itself from
	 * the previous or the new parent children collection.
	 * 
	 * @param parent
	...
	/**
	 * @return <code>true</code> only if one of the ancestors is &lt;pre> tag.
	 *         <code>false</code> otherwise (including case where this node is
	 *         &lt;pre> tag)
	 */
